...
function App() {
  ...
  const clean = () => {
    ...
    if(swapPTagNewlinesSpaces && origTxtType === 'html' && conversionType === 'plain') {
  ...
  const swapPTagNewlinesSpacesOption = origTxtType === 'html' || conversionType === 'html' ? (
    <label>
      <input type="checkbox" checked={swapPTagNewlinesSpaces} onChange={(e) => setSwapPTagNewlinesSpaces(e.target.checked)} />
      Replace newlines in p tags with spaces
    </label>
  ) : null
